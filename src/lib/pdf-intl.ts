...
export async function exportPDFIntl(
  ...
  try {
    ...
    const safeCustomer = (customer.name || "Customer").replace(/[\\/:*?"<>|]/g, "_");
    const safeNo = (quoteMeta.no || "Quotation").replace(/[\\/:*?"<>|]/g, "_");
    const filename = `Quotation_${safeNo}_${safeCustomer}_${quoteMeta.date}.pdf`;
    pdf.save(filename);
  } finally {
    exportEl.remove();
  }
